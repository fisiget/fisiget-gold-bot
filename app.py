...
# ==========================================
# METATRADER TRADING AUTOMATION ENGINE
# ==========================================
...
def execute_market_order(direction: str, price_ask: float, price_bid: float):
    if count_open_positions() > 0:
        st.session_state.last_trade_log = f"⏳ {direction} ignoriert – Position läuft bereits."
        return
    ...
    if result.retcode != mt5.TRADE_RETCODE_DONE:
        st.session_state.last_trade_log = f"❌ Order-Fehler! MT5 Code: {result.retcode} ({result.comment})"
    else:
...
def get_broker_market_data():
    ...
    try:
        # ABSOLUT DIREKTE KOPPLUNG OHNE UMWEGE:
        login_success = mt5.login(62128025, "@A09351531011a@", "PepperstoneUK-Demo")
    except Exception as e:
        st.session_state.connection_status = f"❌ Login-Systemfehler: {str(e)}"
        return None
...
# ==========================================
if live_gold:
    if "BUY" in st.session_state.ki_signal and mathe_signal == "BUY":
        execute_market_order("BUY", st.session_state.broker_buy, st.session_state.broker_sell)
    elif "SELL" in st.session_state.ki_signal and mathe_signal == "SELL":
